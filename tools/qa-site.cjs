...
async function runViewport(browser, name, viewport) {
  ...
  await page.locator("#download [data-close]").click();
  await page.locator('[data-open="audio"]').click();
  const audioDialogVisible = await page.locator("#audio").isVisible();
  const audioStatus = await page.locator("[data-audio-status]").textContent();
  ...
  await context.close();
  return {name,viewport,main,dialogVisible,audioDialogVisible,audioStatus,checked,consoleErrors,pageErrors,badResponses};
}
...
(async () => {
  ...
  try {
    ...
    if (results.some(result => result.main.overflow > 0 || result.main.brokenImages.length || result.main.missingAnchors.length || !result.dialogVisible || !result.audioDialogVisible || result.consoleErrors.length || result.pageErrors.length || result.badResponses.length || result.checked.some(page => page.status !== 200 || page.overflow > 0 || page.brokenImages))) process.exitCode = 1;
  } finally {
    await browser.close();
    server.close();
  }
