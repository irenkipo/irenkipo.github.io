...
13. Run `node tools/qa/e2e-closeout.cjs` as the final user-journey closeout check: reading save/restore and homepage continuation, audiobook chapter/time restore without autoplay plus pause/save on close, EPUB/PDF/audio-ZIP links, canonical LitRes link, intercepted subscription transport/status/event, and one-click unsubscribe token transport/status.
