...
- Initial-load image deferral delta: keep the hero banner and first-book mockup eager, but defer below-the-fold series covers, Book 2 mockup and the large author mark with native lazy loading and async decode. This reduces initial page transfer without changing image files, geometry, visible copy, layout, artwork, typography or palette. Visual QA explicitly forces lazy images to load before pixel comparison so the visual lock remains strict.
